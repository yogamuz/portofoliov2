    ...
    data: {
      label: (
        ...
        >
          ...
          <span className="text-2xl text-gray-300 font-medium text-center whitespace-nowrap">{tool.name}</span>
        </div>
      ),
    },
    style: {
...
function createSkillsEdges(skillsCount) {
  return Array.from({ length: skillsCount }, (_, index) => ({
    ...
    target: 'tech-stack',
    animated: true,
    style: { stroke: '#7DD3FC', strokeWidth: 4 },
  }));
}
      ...
      label: (
        ...
        >
          {tool.icon ? (
            ...
            <img src={tool.iconUrl} alt={tool.name} className="w-8 h-8 flex-shrink-0 hover:opacity-80 transition-opacity duration-300" />
          )}
          <span className=" text-gray-300 font-medium whitespace-nowrap">{tool.name}</span>
        </div>
      ),
    },
    style: {
...
export default function Skills({ showHeader = true, className = '' }) {
  ...
  const mobileToolsNodes = createMobileToolsNodes(tools, isVisible, skills.length);
  const mobileNodes = [...mobileSkillsNodes, mobileCenterNode, ...mobileToolsNodes];
  ...
  return (
    <div ref={sectionRef} className={`w-full flex flex-col ${className}`}>
      ...
      <div className="hidden md:block w-full mx-auto" style={{ maxWidth: '1200px', height: '550px', pointerEvents: 'none' }}>
        <ReactFlow
          nodes={nodes}
          edges={edges}
          fitView
      ...
      <div className="md:hidden w-full mx-auto px-2" style={{ height: '750px' }}>
        <ReactFlow
          nodes={mobileNodes}
          edges={mobileEdges}
          fitView
